...
import styles from '../styles/VolunteerDashboard.module.css'; // Import CSS module
...
const VolunteerDashboard = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      // Categorize events into upcoming and ongoing (suitable for volunteers)
      ...
      const highlightedDates = data.map(event => new Date(event.startDate).toISOString().split('T')[0]);
  ...
  const handleDateClick = (date) => {
    ...
    // Navigate to a detailed view for the selected date (optional)
    navigate(`/event-details/${formattedDate}`);
  };
